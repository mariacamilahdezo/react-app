...
import { selectUserRole } from 'app/store/slices/user-slices/userSlice';
import FuseNavigationHelper from '@fuse/utils/FuseNavigationHelper';
import i18next from 'i18next';
import FuseNavItemModel from '@fuse/core/FuseNavigation/models/FuseNavItemModel';
import FuseUtils from '@fuse/utils';
import navigationConfig from 'app/configs/navigationConfig';
import { selectCurrentLanguageId } from 'app/store/slices/general-slices/i18nSlice';
...
export const updateNavigationItem =
	(id, item) => async (dispatch, getState) => {
		...
		dispatch(
			setNavigation(
				FuseNavigationHelper.updateNavItem(navigation, id, item),
			),
		);
		return Promise.resolve();
	};
